...
class GraphFactoryTests(unittest.TestCase):
    ...
    def test_context_from_parent(self):
        ...
        child = Context.with_parent(parent).new("child")
    ...
    def test_context_from_uuid(self):
        ...
        ctx = Context.from_uuid(project_id)

        self.assertEqual(ctx.id, project_id)
        self.assertEqual(ctx.name, str(project_id))
        self.assertIsNone(ctx.parent)

        row = self._get_graph_row(project_id)
        self.assertIsNotNone(row)
        self.assertEqual(row["graph_id"], str(project_id))
        self.assertEqual(row["name"], str(project_id))
        self.assertIsNone(row["parent_id"])
