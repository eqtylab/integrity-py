...
class TestAsset(unittest.TestCase):
    """Checks that python objects that are wrapped by eqty Assets can still be used like their original python types."""
    ...
    def test_attribute_access_success(self):
        ...
        asset = Dataset.from_object(data)
        self.assertEqual(
            len(asset), 3, "Incorrect Dataset array length"
        )  # Assuming len works on the list
    ...
    def test_modulo_with_compatible_types(self):
        """Test modulo of Dataset.from_object objects with compatible underlying types (integers)."""
        data = Dataset.from_object(10)
        divisor = 3
        result = data % divisor
...
class TestItem:
    model = "The model to use for processing the query, defaults to None."
    content_session_id = "12345"

    def serialize_for_hashing(self):
        return self.model.encode("utf-8")
