...
class Declaration:
    ...
    def finalize(self) -> "Declaration":
        ...
        self.submitted_by = eqty_core_signer.get_active_signer_did_key()

        declaration_json = self.to_json()
        logger.debug(f"Finalizing declaration. {declaration_json}")
        cid = get_cid_for_bytes(declaration_json.encode("utf-8"), True)
        logger.info(f"Declaration CID: {cid}")
        return self

    def cid(self) -> str:
        declartion_json = self.to_json()
        return get_cid_for_bytes(declartion_json.encode("utf-8"))
