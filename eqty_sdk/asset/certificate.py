...
class Certificate(Asset):
    ...
    def from_path(path: Union[str, Path], store: Optional[bool] = None, **kwargs) -> "Certificate":
        ...
        return cast(
            "Certificate", Asset._from_path(ctx, path, AssetType.CERTIFICATE, store, **kwargs)
        )
    ...
    def from_cid(cid: Union[Cid, str], **kwargs) -> "Certificate":
        ...
        if isinstance(cid, Cid):
            return cast(
                "Certificate",
                Asset._from_cid(ctx, cid.cid, AssetType.CERTIFICATE, **kwargs),
            )
        else:
            return cast("Certificate", Asset._from_cid(ctx, cid, AssetType.CERTIFICATE, **kwargs))
    ...
    def from_object(obj: Any, store: Optional[bool] = None, **kwargs) -> "Certificate":
        ...
        return cast(
            "Certificate",
            Asset._from_object(ctx, obj, AssetType.CERTIFICATE, store, **kwargs),
        )

    @feature_gate_when_disabled(FEATURE_FLAGS.GRAPH_IDS)
    def add_attribute(self, **kwargs) -> "Certificate":
        self.add_attribute(**kwargs)
        return self

    @feature_gate_when_disabled(FEATURE_FLAGS.GRAPH_IDS)
    def remove_attribute(self, **kwargs) -> "Certificate":
