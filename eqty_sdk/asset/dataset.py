...
class Dataset(Asset):
    ...
    def from_path(path: Union[str, Path], store: Optional[bool] = None, **kwargs) -> "Dataset":
        ...
        return cast("Dataset", Asset._from_path(ctx, path, AssetType.DATASET, store, **kwargs))
    ...
    def from_cid(cid: Union[Cid, str], **kwargs) -> "Dataset":
        ...
        if isinstance(cid, Cid):
            return cast("Dataset", Asset._from_cid(ctx, cid.cid, AssetType.DATASET, **kwargs))
        else:
            return cast("Dataset", Asset._from_cid(ctx, cid, AssetType.DATASET, **kwargs))
    ...
    def from_object(obj: Any, store: Optional[bool] = None, **kwargs) -> "Dataset":
        ...
        return cast("Dataset", Asset._from_object(ctx, obj, AssetType.DATASET, store, **kwargs))

    @feature_gate_when_disabled(FEATURE_FLAGS.GRAPH_IDS)
    def add_attribute(self, **kwargs) -> "Dataset":
        self.add_attribute(**kwargs)
        return self

    @feature_gate_when_disabled(FEATURE_FLAGS.GRAPH_IDS)
    def remove_attribute(self, **kwargs) -> "Dataset":
